...
public class StudentRestController {
    ...
    public ResponseEntity<String> createStudent(@Valid @RequestBody Student student, BindingResult result) {
        if (result.hasErrors()) {
            ...
            for (int i = 0; i < result.getAllErrors().size(); i++) {
                errors.append(result.getAllErrors().get(i).toString());
    ...
    public ResponseEntity<String> updateStudent(@PathVariable Long id, @Valid @RequestBody Student updateStudent, BindingResult result) {
        if (result.hasErrors()) {
            StringBuilder errors = new StringBuilder();
            for (int i = 0; i < result.getAllErrors().size(); i++) {
                errors.append(result.getAllErrors().get(i).toString());
        ...
        return new ResponseEntity<>("Modification de l'étudiant ok" , HttpStatus.ACCEPTED);
    }
